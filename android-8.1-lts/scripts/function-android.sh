...
build_application_mk() {
  ...
  #
  # This used to be a hand-kept list of eleven libraries known to be C++ (x265,
  # tesseract, openh264, snappy, rubberband, zimg, srt, chromaprint, libilbc,
  # whisper, plus a custom-library flag). That list was right while the link line
  # was conditional too. It stopped being right when linking became unconditional --
  # the very change made after the `audio` tier crashed for exactly this reason --
  # and nobody updated the second half. The `min` and `https` tiers enable none of
  # the eleven, so from that day on they shipped .so files that could not load.
  #
...
get_common_linked_libraries() {
  ...
  ffmpeg)
    ...
    # function used to only add for srt/tesseract/whisper/x265. Previously
    # this went unnoticed because the paid Pro tier always also enabled
    # x265, which coincidentally pulled -lc++_shared in. The free "audio"
    # tier (libilbc, no x265) exposed it as a genuine dlopen crash -- CI's
    # static/binary checks don't catch missing dynamic-link deps, only a
